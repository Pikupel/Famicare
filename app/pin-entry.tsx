import { useState, useEffect } from 'react';
import { View, Text, TouchableOpacity, StyleSheet, Alert } from 'react-native';
import { useRouter } from 'expo-router';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useAuthStore } from '../src/stores/useAuthStore';
import { colors } from '../src/theme/colors';
import { typography } from '../src/theme/typography';
import { spacing } from '../src/theme/spacing';

export default function PinEntryScreen() {
  const router = useRouter();
  const role = useAuthStore((s) => s.role);
  const [pin, setPin] = useState('');
  const [savedPin, setSavedPin] = useState('');

  useEffect(() => {
    AsyncStorage.getItem('famicare_pin').then(pin => { if (pin) setSavedPin(pin); });
  }, []);

  const handleDigit = (d: string) => {
    const newPin = pin + d;
    setPin(newPin);
    if (newPin.length === 4) {
      if (newPin === savedPin) {
        router.replace(role === 'caregiver' ? '/caregiver' : '/home');
      } else {
        Alert.alert('Hatalı PIN', 'Tekrar deneyin');
        setPin('');
      }
    }
  };

  const handleDelete = () => setPin(pin.slice(0, -1));

  const handleLogout = () => {
    useAuthStore.getState().logout();
    router.replace('/welcome');
  };

  return (
    <View style={{ flex: 1, backgroundColor: colors.background, justifyContent: 'center', paddingHorizontal: spacing.xl }}>
      <Text style={{ ...typography.h2, color: colors.text, textAlign: 'center', marginBottom: spacing.sm }}>PIN ile Giriş</Text>
      <Text style={{ ...typography.body, color: colors.textSecondary, textAlign: 'center', marginBottom: spacing.xxl }}>Hızlı giriş için PIN kodunuzu girin</Text>

      <View style={{ flexDirection: 'row', justifyContent: 'center', gap: spacing.md, marginBottom: spacing.xxl }}>
        {[0, 1, 2, 3].map(i => (
          <View key={i} style={[styles.dot, { backgroundColor: pin.length > i ? colors.primary : colors.border }]} />
        ))}
      </View>

      <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: spacing.sm }}>
        {['1','2','3','4','5','6','7','8','9','', '0','⌫'].map((d, i) => (
          d === '' ? <View key={i} style={{ width: 80, height: 80 }} /> :
          <TouchableOpacity key={i} style={styles.key} onPress={() => d === '⌫' ? handleDelete() : handleDigit(d)}>
            <Text style={{ ...typography.h1, color: colors.text }}>{d}</Text>
          </TouchableOpacity>
        ))}
      </View>

      <TouchableOpacity onPress={handleLogout} style={{ marginTop: spacing.xl, alignItems: 'center', minHeight: 48, justifyContent: 'center' }}>
        <Text style={{ ...typography.body, color: colors.textLight }}>Farklı bir hesapla giriş yap</Text>
      </TouchableOpacity>
    </View>
  );
}

const styles = StyleSheet.create({
  dot: { width: 16, height: 16, borderRadius: 8 },
  key: { width: 80, height: 80, borderRadius: 40, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 2 },
});
